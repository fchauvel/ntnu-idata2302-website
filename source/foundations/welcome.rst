...
Computation, algorithms, and data structures. From there, I want to go
through a few some practicalities, including learning outcomes,
expectations and supporting material, and examinations.
...
:math:`\skull` stands for danger or death. Numbers are another example
...
Figure `[fig:school-addition] <#fig:school-addition>`__ shows the setup
I would use to add to 967. Here are the steps I would follow:
...
TAOCP :raw-latex:`\cite[p. 5--6]{knuth1978}` (see
Section `6 <#sec:resources>`__), where he specifies the four following
properties:
...
-  An algorithm is *well-defined*, and each step is non-ambiguous. In
   ...
   over as shown on
   Figure `[fig:school-addition-table] <#fig:school-addition-table>`__.
   [-1.5cm]
...
In our addition example (see
Figure `[fig:school-addition] <#fig:school-addition>`__) we use a “grid”
that stores all the data, including the two given numbers (the inputs),
the result (output) and the carryovers (intermediate results). This grid
...
get rid of the grid. The very same apply to algorithms: Appropriate data
structure is the key to their efficiency.
...
Figure `1 <#fig:flowchart>`__. In a flowchart, the steps of an algorithm
are shown as boxes connected by arrows. The flowchart syntax
distinguishes between various type of steps such as processes, document,
decisions, references, etc using different shapes.
Figure `1 <#fig:flowchart>`__ only use “processes” (shown as rectangles)
and decisions (shown as diamond). A flowchart makes the control
structure (loops and decisions) very explicit, but the graphical syntax
is quite space consuming and may not scale to complex algorithms.
...
.. container:: algorithm
   ...
   Setup :math:`x` and :math:`y` into a grid Place your finger under the
   right most column
...
relevant. The two first weeks focus on the basics: What is an algorithm,
what is a program, how does a computer executes a program, how to
measure time and memory consumption? etc. Then we apply these again and
again on basic algorithms and data structures: Records, arrays, lists,
...
Anyone can join and succeed in this course: There is no prerequisite. I
will use some high-school maths (functions, sets, probabilities,
summations, boolean logic), which I recall in a separate appendix. The
plan is to explain everything we need as we go. This is *not* a maths
class.
...
that algorithms and data structures apply to any programming language. I
provide written lecture notes for the most important lectures, so you
...
*This content is hard*—I cannot stress this enough. Some of which are
non-trivial. Yet, I made everything optional, but the final examination.
I see this course is an opportunity to study algorithms and data
structures. Here is what I *strongly* recommend:
...
As you can see in Table `[tab:syllabus] <#tab:syllabus>`__, this course
is “content-heavy”. I don’t think one can master the whole thing in a
week or so before the final examination. A strongly recommend you spread
your work over the 13 weeks.

What can you expect from me?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

I am there three times a week for all lectures and lab sessions. Feel
free to come and ask questions. I try to be available and responsive, by
email especially. I try to reply within 24 hours, but bear in mind that
I am also a full time engineer. Expect answers outside office hours.
...
:raw-latex:`\cite{atto1974,melhorn2008,levitin2011,weiss2014,skiena2020}`,
videos, online courses, tutorials or blog posts on algorithms and data
structures. This course in an introduction: I am not presenting anything
new or innovative. Let me know if the angle I have chosen is unclear or
does not fit your background. We can look at other resources where the
same content is taken from a different perspective.

*I do* **not** *follow any textbook* in particular, but there are a few
“references” textbooks, whose authors are leading authorities. Feel free
to check them if you need a different treatment:

-  “The Art of Computer Programming” (aka TAOCP) by Donald E. Knuth.
   This is *the* reference. Five volumes so far:
   Fundamentals :raw-latex:`\cite{knuth1978}`,
   semi-numerical :raw-latex:`\cite{knuth1997}`, sorting and
   searching :raw-latex:`\cite{knuth1998}`, combinatorial
   algorithms :raw-latex:`\cite{knuth2011}`. I found the treatment very
   detailed and oriented towards mathematicians. Beyond the scope of
   this course.

-  “Introduction to Algorithms” by Thomas H. Cormen et
   al :raw-latex:`\cite{cormen2009}` is another famous reference. It
   covers most of the topic I selected and many more, except maybe
   correctness.

-  “Algorithms” by Robert Sedgewick :raw-latex:`\cite{sedgewick2014}` is
   another well-known textbook, which is used in some very successful
   online courses. Does not cover the whole syllabus.

-  “Data structures and algorithms in Java” by Michael T. Goodrich and
   Roberto Tamassia :raw-latex:`\cite{goodrich2014}` fits very well this
   course. Most content we will cover is also treated there.

There are also online course on platforms such as
`Coursera <https://www.coursera.org>`__ or `EdX <https://www.edx.org>`__
...
`GeeksForGeeks <www.geeksforgeeks.com>`__ or
`TutorialsPoint <www.tutorialspoint.com>`__. Please keep a critical eye
